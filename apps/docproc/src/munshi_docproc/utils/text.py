...
ROMAN_NUMERAL_RE = r"[ivxlcdmIVXLCDM]+"
...
def normalize_superscript_refs(text: str) -> str:
    """Normalize superscript markers to caret notation (^N)."""
    # Convert HTML/LaTeX superscripts to ^N notation rather than parenthesized (N)
...
def normalize_for_frequency(line: str) -> str:
    ...
    Casefold, collapse whitespace, strip page numbers (Arabic and Roman).
    """
    line = line.casefold()
    # Strip leading/trailing page numbers (Arabic digits or Roman numerals)
    line = re.sub(rf"^(?:\d+|{ROMAN_NUMERAL_RE})\s*", "", line)
    line = re.sub(rf"\s*(?:\d+|{ROMAN_NUMERAL_RE})$", "", line)
    # Collapse whitespace
